...
const ShipmentsPageBranchOffice = () => {
  ...
  const fetchShipments = async () => {
    try {
      ...
      if (!branchId) {
        console.error('Branch ID not found in user data:', userData);
        throw new Error('Branch ID not found in user data');
      }

      console.log('Making API request with filter:', filter);
      const response = filter === 'all' 
        ? await apiService.getShipmentsByBranch(branchId)
      ...
      if (response.data && response.data.data) {
        ...
        console.warn('No shipments data in response:', response);
        setShipments([]);
      }
      setError('');
    } catch (err) {
...
export default ShipmentsPageBranchOffice;
