...
const PickupPageBranchOffice = () => {
    ...
    useEffect(() => {
        // Get branch ID from user data in localStorage
        const userData = JSON.parse(localStorage.getItem('user') || '{}');
        console.log('User Data from localStorage:', userData);
        if (userData.branch && userData.branch.branch_id) {
            console.log('Setting branch ID:', userData.branch.branch_id);
            setBranchId(userData.branch.branch_id);
        } else {
    ...
    useEffect(() => {
        console.log('useEffect triggered - branchId:', branchId, 'filter:', filter);
        if (branchId) {
            fetchPickups();
        } else {
            console.log('No branch ID available, skipping fetchPickups');
        }
    }, [filter, branchId]);
    ...
    const fetchPickups = async () => {
        try {
            setLoading(true);
            console.log('Starting fetchPickups - branchId:', branchId, 'filter:', filter);
            
            let response;
            if (filter === 'all') {
                console.log('Fetching all pickups for branch:', branchId);
                response = await apiService.getPickupsByBranch(branchId);
            } else {
                console.log('Fetching filtered pickups with status:', filter);
                response = await apiService.getFilteredPickupRequests(filter);
            }
...
export default PickupPageBranchOffice;
