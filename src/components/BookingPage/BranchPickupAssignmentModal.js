...
const PickupAssignmentModal = ({ onClose, onSubmit, bookingNumber }) => {
  ...
  const ownBranch = user?.branch;

  const [booking, setBooking] = useState(null);
  const [formData, setFormData] = useState({
    branch_id: ownBranch?.branch_id || '',
    executive_id: '',
    pickup_date: '',
    pickup_time: '',
    notes: ''
  ...
  // Auto-select own branch when modal opens
  useEffect(() => {
    if (ownBranch?.branch_id) {
      setFormData(prev => ({
        ...prev,
        branch_id: ownBranch.branch_id
      }));
              ...
              <div className="form-section">
                ...
                <div className="form-group">
                  ...
                  <select
                    ...
                    disabled={true}
                  >
                    <option value={ownBranch?.branch_id || ""}>
                      {ownBranch
                        ? `${ownBranch.branch_name} - ${ownBranch.city || ""}, ${ownBranch.state || ""}`
                        : "No branch found"}
                    </option>
                  </select>
